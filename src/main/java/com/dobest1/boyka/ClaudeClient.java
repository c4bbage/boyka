...
this(config,prompt,null);
    ...
    public String sendMessage(String userMessage,String context, List<Tool> availableTools) throws IOException {
        ...
        Request request = new Request.Builder()
                ...
                .build();
        BoykaAILogger.info("sendMessage Request: " + requestBody.toString());
    ...
    public String sendMessageNoHistory(String systemPrompt,String userMessage,String context, List<Tool> availableTools) throws IOException {
        ...
        Request request = new Request.Builder()
                ...
                .addHeader("anthropic-version", config.getAnthropicVersion())
                .build();
        BoykaAILogger.info("sendMessageNoHistory Request: " + requestBody.toString());
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
...
//                conversationHistory.add(new Message("assistant", block.text));
                ...
                conversationHistory.add(new Message("assistant", Arrays.asList(block)));
                String toolResult = executeToolCall(block);
                finalResponse.append("Tool used: ").append(block.name)
                        .append("\nResult: ").append(toolResult).append("\n");
                conversationHistory.add(new Message("user", Arrays.asList(
                ...
                finalResponse.append("AI response: ").append(claudeResponseToTool).append("\n");
            }
        }
...
//        toolResultContent.addProperty("tool_use_id", toolUseId);
//        toolResultContent.addProperty("content", toolResult);
//        content.add(toolResultContent);
//        toolResultMessage.add("content", content);
//        messages.add(toolResultMessage);
        ...
        Request request = new Request.Builder()
                ...
                .addHeader("anthropic-version", config.getAnthropicVersion())
                .build();
        BoykaAILogger.info("sendToolResultToClaude Request: " + requestBody.toString());
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
